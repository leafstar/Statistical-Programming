...
## In order to find valid steps and compute the gradients of the following functions were created:
...
get_grad <- function(f,...,theta,eps){                       ## defines get_grad function 
  if(is.null(attributes(f(theta, TRUE,...))$gradient)){      ## if f does not supply a gradient attribute computes gradient by user 
    ...
    for (i in 1:length(theta)) {                             ## calculates for loops over parameters
      theta1 <- theta; theta1[i] <- theta1[i] + eps          ## increase theta by a small distance of eps
      grad[i] <- (f(theta1,...) - f(theta,...))/eps          ## numerically calculate the slope
    }
...
get_step_size <- function(f,...,theta,d){                    ## defines get_step_size function
  ...
  g = get_grad(f = f,...,theta = theta,eps = eps)            ## Create the gradient
  gamma = c1*sum(g*d)                                        ## gamma is the minimizing progress we wish to make
  
  while(iter <= max.iter){                                   ## iterate until the number max of iterations is reached
    iter = iter + 1                                          ## increment iter 
    theta.prime = theta + alpha*d                            ## calculate theta.prime, which is the next theta
    g.prime = get_grad(f = f,..., theta = theta.prime, eps = eps)  ## calculate gradient at next theta(theta.prime) 
  ...
  alpha ## return the step size we found.
}
...
bfgs <- function(theta,f,...,tol=1e-5,fscale=1,maxit=100){ ## define the function "bfgs" with 5 inputs
  theta.k = theta             ## set theta.k to the initial values for the optimization parameters theta 
  B.k <- diag(length(theta))  ## initialize the inverse of approximation of hessian to be the identity matrix
  f0 = f(theta.k,...)         ## calculate f0 as the first objective value
  eps = 1e-7  ## finite difference interval
  g.k = get_grad(f=f , ...,theta = theta.k,eps =  eps) ## get the gradient at theta
  ...
  iter = 0 ## initialize iter, which is a counter of number of iterations.
  ...
  ## outline:
  ##  1. calculate current objective and gradient.
  ##  2. calculate the direction d as the negative of the product of inverse hessian and gradient.
  ##  3. find a valid step size at current theta.
  ##  4. get new theta.kprime based on the step size. 
  ##  5. calculate the gradient g.kprime at theat.kprime
  ##  6. calculate s.k, y.k as the difference between theta's and gradients,
  ##     which are (theta.kprime - theta.k), and (g.kprime - g.k) respectively.
  ##  7. calculate rho.k by taking the dot product
  ##  8. update the inverse of approximation of hessian based on s.k, y.k, and rho.k
  ##  9. update theta.k, B.k and increment iter.
  while(max(abs(g.k)) >= (abs(f0)+fscale)*tol && iter <= maxit){  
    
    f0 = f(theta.k,...)  ## calculate current objective value
    
    g.k = get_grad(f = f,..., theta = theta.k, eps =  eps) ## re calculate the gradient
    
    d = -B.k %*% g.k  ## calculate the direction as the negative of the product of inverse hessian and gradient.
    ...
    ## get the gradient kprime from the get_grad function
    g.kprime = get_grad(f = f,..., theta = theta.kprime, eps = eps)
    
    ## calculate s.k as the difference of theta.kprime and theta.k
    s.k = theta.kprime - theta.k 
    
    ## calculate y.k as the difference between the gradients.
    y.k = g.kprime - g.k
    
    ## calculate rho.k inverse as the dot product of s.k and y.k
    rho.k.i = sum(s.k*y.k)
  ...
  ## if maximum of iterations has been reached without convergence, raise a warning.
