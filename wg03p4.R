...
## (1) if a step leads to a non-finite objective value the step length is reduced.
## Step length reduces the objective function and satisfies the second Wolfe condition (use c2 = 0.9). 
...
get_step_size <- function(f,...,theta,d){                    ## defines get_step_size function
  ...
  g = get_grad(f = f,...,theta = theta,eps = eps)            ## Create the gradiente 
  gamma = c1*sum(g*d)                                        ## for cond1 we get the value of gamma
  cat("alphagamma",gamma,"\n")                               ## print the value of gamma
  while(iter <= max.iter){                                   ## iterate until the number max of iterations is reached
    iter = iter + 1                                          ## Sum the iter at each step
    g.prime = get_grad(f = f,..., theta = theta.prime, eps = eps)       ## calculate g.prime based on the funtion, theta.prime and eps value
    if(f(theta.prime,...) < f(theta,...) + alpha * gamma && g.prime %*% d /(g %*% d) <= c2){   
      break
    }
    
    if(f(theta.prime,...) >= f(theta,...) + alpha* gamma){
      alpha = alpha/2                                        ## update alpha if this condition is true
    }
    
    if(g.prime %*% d /(g %*% d) > c2){                       ## if g.prime x direction / gradiente x direction bigger than second wolf condition parameter
      alpha = 1.1 * alpha                                    ## update alpha if the previous condition is true
    }
    theta.prime = theta + alpha*d                            ## update theta if this condition is true
  }
  
  if(f(theta.prime,...) >= f(theta,...)){                    ## if theta prime bigger than theta
    alpha=1                                                  ## set alpha =1 
    warning("we cannot find a valid step size")              ## and print a warning 
  }
