...
## (1) Simulate the pandemic and record the number of daily infections for three groups:
##     1) Whole population; 
##     2) Cautious group - 10% of the population with the lowest individual transmission probabilities; 
##     3) A random sample of 0.1% of the population n=5.5e6.
## (2) Produce a plot showing how the daily infection trajectories compare between
##     the whole population,the ‘cautious 10%’ and the 0.1% random sample. 
## (3) Run 10 replicate simulations.
## (4) Produce plots for 10 replicate simulations showing daily infection trajectories for each groups and their variability.


## Step (1):Simulate the pandemic and record the number of daily infections for three groups

## Implement Stochastic Simulation Model as a function; 
## Started the epidemic by setting 10 randomly chosen people to the Exposed state and 
## Simulated for 150 days. Inputs are the (hyper)parameters and the output is the number of people in each state each day.
simulation<-function(n = 5.5e6, ne = 10, nt = 150, gamma = 1/3, delta = 1/5, lambda = 0.4/n){ 
  ...
  cautious_group = which(beta <= quantile(beta, 0.1))            ## get 10% of the most cautious group
  random_group = sample(1:n, n/1000)                             ## create a random sample of 0.1% of the population
  ...
  S_ind = 1:n; E_ind = initial_exposure; I_ind = c(); R_ind = c(); ## initialize four numerical indices vector of 4 states
  for (i in 2:nt){                                               ## loop over 150 days
    u_stoe <- runif(length(S_ind))  ## uniform random deviates used to determine whether a individual's state should change 
    ...
    ## change individuals' state and also update the corresponding indices vector                                
    ...
    ## Record the number of new infections each day for the 3 chosen groups
  ...
  list(I1=I1,I2=I2,I3=I3)
}

#Step (2):Produce a plot showing how the daily infection trajectories compare between 
## the whole population, the ‘cautious 10%’ and the 0.1% random sample.
...
legend("topleft", legend=c("whole population", "cautious group", "random 0.1% group"),
...
for (i in 1:10){                                                   ## indicate in the for loop that the simulation has to run 10 times
  epi10[[length(epi10)+1]] <- simulation()                         ## Store the outputs in a list called epi10
}
...
for (i in 1:10) {
  lines(1:xlim,log(epi10[[i]]$I2), col = 4)
}
...
legend("topleft", legend=c("whole population", "cautious group", "random 0.1% group"),
       col=c("black", "blue","red"), lty=1, lwd=3,cex =0.65)            
...
## representative. This conclusion holds for the ZOE symptom tracker app results as well, which shows the similar patterns as 
