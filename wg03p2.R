...
## For that purpose we split the population by 4 categories 1) Susceptible 2) Exposed 3) Infective and 4) Recovered and do following steps:
  ## (1) Show daily infection trajectories for three groups 1) Whole population 2) Cautious group - 10% of the population with the lowest transmission
  ## rates and 3) Random sample of 0.1% of the population.
  ...
  ## (4) Produce three plots for 10 replicate simulations showing daily infection trajectories for each groups.

   ## Step(1):Show daily infection trajectories for three groups 1) Whole population 2) Cautious group - 10% of the population with the lowest transmission
   ## rates and 3) Random sample of 0.1% of the population.
## Defined Stochastic Simulation Model function
## Started the epidemic by setting 10 randomly chosen people to the Exposed state and Simulated for 100 days.
...
simulation<-function(n = 5.5e6, ne = 10, nt = 150, gamma = 1/3, delta = 1/5, lambda = 0.4/n){ 
  ...
  initial_exposure = sample(1:n, ne) ## start with the sample 10 people in E state
  x[initial_exposure] = 1 ## creat some infectives
  I1 <- I2 <- I3 <- rep(0, nt) ## set up storage for number of infective people for 3 groups
  for (i in 2:nt){ ## loop over days
    u <- runif(n) ## uniform random deviates
    exposed_prob = lambda * beta * sum(beta[x==2]) ## Set probability of uninfected person of being infected and entering (E) state
    x[x==2 & u<delta] <- 3 ## I -> R with prob delta
    x[x==1 & u<gamma] <- 2 ## E -> I with prob gamma
    x[x==0 & u<exposed_prob] <- 1 ## S -> E with prob 'exposed_prob'

    ## record the number of new infections each day for 3 groups
    I1[i] <- sum(x==2); ## record for total population
    I2[i] <- sum(x[cautious_group]==2); ## record for cautiouse group
    I3[i] <- sum(x[random_group]==2); ## record for random sample of 0.1% of the population
  }
...
##(4.2) Plot for the cautious group-10% of the population
